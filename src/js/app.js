...
function iniciarApp() {
  ...
  botonesPaginador(); //Agrega o quita los bottones del paginador
  paginaSiguiente();
  paginaAnterior();
...
function mostrarSeccion() {
  //Ocultar la seccion que tiene la clase de mostrar
...
function seleccionarServicio(servicio) {
  ...
  //Identificar al elemento al que da click
...
function seleccionarFecha() {
  ...
  inputFecha.addEventListener("input", function (e) {
    ...
    //DIa utc no hay cita los dias sabado y domingo, en utc 6=sabado 0=domingo
    if ([6, 0].includes(dia)) {
      e.target.value = "";
...
function mostrarAlerta(mensaje, tipo, elemento, desaparece = true) {
  ...
  if (desaparece) {
    //En 3 segundos el mensaje de alerta se eliminará
    setTimeout(() => {
      alerta.remove();
    }, 3000);
  }
...
async function reservarCita() {
  ...
  try {
    ...
    Swal.fire({
      icon: "error",
      title: "Error",
      text: "Algo extraño sucedió al intentar guardar la cita",
      button: "Aceptar"
    });
  }
